...
public class BoardDao {
    ...
    private BoardDao(){
        ...
        try {
            ...
            connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/mydb1126","root","1234");
            // [3] 확인
    ...
    public boolean boardDelete(int D) {
        try {
            ...
            ps.setInt(1, D);
            ps.executeUpdate();
            int result = ps.executeUpdate(); // delete 실행 후 delete 레코드 개수를 반환,result(삭제된 레코드 수)
            if (result == 1) {
                return true;
            }
        } catch (SQLException e) {
